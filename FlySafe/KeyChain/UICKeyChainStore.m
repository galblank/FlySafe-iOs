...
+ (NSData *)dataForKey:(NSString *)key service:(NSString *)service accessGroup:(NSString *)accessGroup
{
    ...
    {
        NSLog(@"[UICKeyChainStore] -dataForKey- status: %d", status);
        return(nil);
	}
...
+ (BOOL)setData:(NSData *)data forKey:(NSString *)key service:(NSString *)service accessGroup:(NSString *)accessGroup
{
    ...
    NSLog(@"[UICKeyChainStore] -setData-forKey- status: %d", status);
	...
	if (status == errSecSuccess)
    {
        if (data)
        {
            ...
            NSLog(@"[UICKeyChainStore] -setData-forKey-errSecSuccess- status: %d", status);
            if (status != errSecSuccess)
            {
                return NO;
            }
    ...
    else if (status == errSecItemNotFound)
    {
        ...
        NSLog(@"[UICKeyChainStore] -setData-forKey-errSecItemNotFound- status: %d", status);
...
- (id)initWithService:(NSString *)s accessGroup:(NSString *)group
{
    ...
    if (self) {
        ...
        [query setObject:(__bridge id)kSecMatchLimitAll forKey:(__bridge id)kSecMatchLimit];
        [query setObject:(id)kCFBooleanTrue forKey:(__bridge id)kSecReturnAttributes];
        // Set the permissins on the KeyChain. Using the device only will allow a new UDID to be generated if the
        // user uses a different device (like if they restore from a backup onto a new device).
